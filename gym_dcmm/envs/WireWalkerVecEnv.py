...
class WireWalkerVecEnv(gym.Env):
    ...
    def __init__(
        ...
        self.object_id = mujoco.mj_name2id(
            self.WireWalker.model, mujoco.mjtObj.mjOBJ_GEOM, self.object_name
        )
                ...
                "arm": spaces.Dict(
                    {
                        ...
                        "joint_pos": spaces.Box(
                            low=np.array(
                                [
                                    self.WireWalker.model.jnt_range[i][0]
                                    for i in range(9, 15)
                                ]
                            ),
                            high=np.array(
                                [
                                    self.WireWalker.model.jnt_range[i][1]
                                    for i in range(9, 15)
                                ]
                            ),
                            dtype=np.float32,
    ...
    def random_object_pose(self):
        ...
        self.object_static_time = np.random.uniform(
            WireWalkerCfg.object_static[0], WireWalkerCfg.object_static[1]
        )
        # Random Quaternion
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--task", type=str, default="Tracking", help="Task for the WireWalkerEnv"
    )
    parser.add_argument(
